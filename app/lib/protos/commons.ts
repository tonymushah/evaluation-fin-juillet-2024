...
class Date$Type extends MessageType<Date> {
	...
	internalBinaryWrite(
		...
		/* uint32 jour = 1; */
		if (message.jour !== 0) writer.tag(1, WireType.Varint).uint32(message.jour);
		/* uint32 mois = 2; */
		if (message.mois !== 0) writer.tag(2, WireType.Varint).uint32(message.mois);
		/* uint32 annee = 3; */
		if (message.annee !== 0) writer.tag(3, WireType.Varint).uint32(message.annee);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
...
class Etudiant$Type extends MessageType<Etudiant> {
	...
	create(value?: PartialMessage<Etudiant>): Etudiant {
		...
		message.prenom = '';
		message.age = 0;
		if (value !== undefined) reflectionMergePartial<Etudiant>(this, message, value);
		return message;
	}
	internalBinaryRead(
		reader: IBinaryReader,
		...
		while (reader.pos < end) {
			...
			switch (fieldNo) {
				...
				case /* mg.tonymushah.evalfjuil24.Date date_naissance */ 4:
					message.dateNaissance = Date.internalBinaryRead(
						reader,
						reader.uint32(),
						options,
	...
	internalBinaryWrite(
		...
		/* string numero = 1; */
		if (message.numero !== '') writer.tag(1, WireType.LengthDelimited).string(message.numero);
		/* string nom = 2; */
		if (message.nom !== '') writer.tag(2, WireType.LengthDelimited).string(message.nom);
		/* string prenom = 3; */
		if (message.prenom !== '') writer.tag(3, WireType.LengthDelimited).string(message.prenom);
		/* mg.tonymushah.evalfjuil24.Date date_naissance = 4; */
		if (message.dateNaissance)
			Date.internalBinaryWrite(
				message.dateNaissance,
				writer.tag(4, WireType.LengthDelimited).fork(),
				options
			).join();
		/* uint32 age = 5; */
		if (message.age !== 0) writer.tag(5, WireType.Varint).uint32(message.age);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
...
class Matiere$Type extends MessageType<Matiere> {
	...
	create(value?: PartialMessage<Matiere>): Matiere {
		...
		message.nom = '';
		message.credits = 0;
		if (value !== undefined) reflectionMergePartial<Matiere>(this, message, value);
		return message;
	}
	internalBinaryRead(
		reader: IBinaryReader,
	...
	internalBinaryWrite(
		...
		if (message.nom !== '') writer.tag(2, WireType.LengthDelimited).string(message.nom);
		/* uint32 credits = 3; */
		if (message.credits !== 0) writer.tag(3, WireType.Varint).uint32(message.credits);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
...
class ReleveNoteUnit$Type extends MessageType<ReleveNoteUnit> {
	constructor() {
		super('mg.tonymushah.evalfjuil24.ReleveNoteUnit', [
			...
			{ no: 2, name: 'valeur', kind: 'scalar', T: 13 /*ScalarType.UINT32*/ },
			{
				...
				name: 'status',
				kind: 'enum',
				T: () => ['mg.tonymushah.evalfjuil24.ReleveNoteStatus', ReleveNoteStatus]
			}
		]);
	}
	create(value?: PartialMessage<ReleveNoteUnit>): ReleveNoteUnit {
		...
		message.valeur = 0;
		message.status = 0;
		if (value !== undefined) reflectionMergePartial<ReleveNoteUnit>(this, message, value);
		return message;
	}
	internalBinaryRead(
		reader: IBinaryReader,
		...
		while (reader.pos < end) {
			...
			switch (fieldNo) {
				case /* mg.tonymushah.evalfjuil24.Matiere matiere */ 1:
					message.matiere = Matiere.internalBinaryRead(
						reader,
						reader.uint32(),
						options,
						message.matiere
	...
	internalBinaryWrite(
		...
		if (message.matiere)
			Matiere.internalBinaryWrite(
				message.matiere,
				writer.tag(1, WireType.LengthDelimited).fork(),
				options
			).join();
		/* uint32 valeur = 2; */
		if (message.valeur !== 0) writer.tag(2, WireType.Varint).uint32(message.valeur);
...
class ReleveNote$Type extends MessageType<ReleveNote> {
	...
	create(value?: PartialMessage<ReleveNote>): ReleveNote {
		...
		message.semestre = 0;
		message.status = 0;
		if (value !== undefined) reflectionMergePartial<ReleveNote>(this, message, value);
		return message;
	}
	internalBinaryRead(
		reader: IBinaryReader,
	...
	internalBinaryWrite(
		...
		for (let i = 0; i < message.notes.length; i++)
			ReleveNoteUnit.internalBinaryWrite(
				message.notes[i],
				writer.tag(1, WireType.LengthDelimited).fork(),
				options
			).join();
		/* uint64 credits = 2; */
		if (message.credits !== 0n) writer.tag(2, WireType.Varint).uint64(message.credits);
		/* uint32 semestre = 3; */
		if (message.semestre !== 0) writer.tag(3, WireType.Varint).uint32(message.semestre);
